...
13. 对富文本或 HTML 条目执行纯文本复制/粘贴按钮，并用 `⌘⇧C` / `⌘⇧↩` 验证会去除格式。
14. 在未授权辅助功能时，确认自动粘贴会降级为复制并提示。
15. 当历史记录引用的媒体数据缺失时，复制、粘贴或置顶快捷键粘贴应提示无法恢复内容，而不是静默失败。
16. 在设置中执行导出备份，确认备份 `Blobs` 只包含历史实际引用的媒体文件，再合并导入一次备份，确认历史可恢复；如果备份缺少 `Blobs` 中被历史引用的媒体文件，应提示导入失败。
17. 合并导入包含图片或富文本的备份后，确认导入条目的预览和粘贴内容仍对应备份中的原始 blob。
18. 如执行覆盖导入，确认会先出现二次确认；确认后当前历史会被备份内容替换，空历史备份应清空当前历史。
