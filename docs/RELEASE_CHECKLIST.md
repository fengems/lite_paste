...
14. 在设置中执行导出备份，确认备份 `Blobs` 只包含历史实际引用的媒体文件，再合并导入一次备份，确认历史可恢复；如果备份缺少 `Blobs` 中被历史引用的媒体文件，应提示导入失败。
15. 合并导入包含图片或富文本的备份后，确认导入条目的预览和粘贴内容仍对应备份中的原始 blob。
16. 如执行覆盖导入，确认会先出现二次确认；确认后当前历史会被备份内容替换，空历史备份应清空当前历史。
