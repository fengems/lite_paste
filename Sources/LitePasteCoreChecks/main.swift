...
func checkICloudBackupService() {
  let directory = FileManager.default.temporaryDirectory
    .appending(path: "LitePasteICloudBackup-\(UUID().uuidString)", directoryHint: .isDirectory)
  let appDirectory = directory.appending(path: "AppData", directoryHint: .isDirectory)
  let iCloudContainer = directory.appending(path: "iCloudContainer", directoryHint: .isDirectory)
  ...
  do {
    ...
    expect(
      exportedURL.lastPathComponent.hasPrefix("LitePaste-") && exportedURL.pathExtension == "litepastebackup",
    ...
    expect(summaryAfterSecondExport.backupCount == 1, "iCloud backup should keep only the latest backup")
    expect(
      summaryAfterSecondExport.latestBackupURL?.standardizedFileURL.path == newerExportedURL.standardizedFileURL.path,
